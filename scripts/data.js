window.initialInventory = [
  {
    id: "aurora-gt",
    make: "Aurora",
    model: "GT",
    year: 2023,
    price: 68900,
    mileage: 8500,
    bodyType: "Coupe",
    fuel: "Hybrid",
    transmission: "Automatic",
    location: "San Francisco, CA",
    color: "Midnight Blue",
    image: "https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Advanced driver assist", "0-60mph in 3.8s", "Adaptive air suspension"],
  },
  {
    id: "zenith-suv",
    make: "Zenith",
    model: "LX",
    year: 2022,
    price: 55950,
    mileage: 14200,
    bodyType: "SUV",
    fuel: "Gasoline",
    transmission: "Automatic",
    location: "Austin, TX",
    color: "Pearl White",
    image: "https://images.unsplash.com/photo-1549921296-3ecf9a5c63c5?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Panoramic roof", "Hands-free tailgate", "360° camera"],
  },
  {
    id: "solstice-sedan",
    make: "Solstice",
    model: "Premier",
    year: 2021,
    price: 38995,
    mileage: 21500,
    bodyType: "Sedan",
    fuel: "Hybrid",
    transmission: "Automatic",
    location: "Denver, CO",
    color: "Graphite Grey",
    image: "https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Heads-up display", "Heated & cooled seats", "Wireless charging"],
  },
  {
    id: "element-hatch",
    make: "Element",
    model: "Pulse",
    year: 2024,
    price: 32990,
    mileage: 3200,
    bodyType: "Hatchback",
    fuel: "Electric",
    transmission: "Automatic",
    location: "Seattle, WA",
    color: "Copper Orange",
    image: "https://images.unsplash.com/photo-1579439228777-7a7b95fb0ac1?auto=format&fit=crop&w=1200&q=80",
    highlights: ["300-mile range", "Fast charging", "Premium sound"],
  },
  {
    id: "venture-trail",
    make: "Venture",
    model: "TrailPro",
    year: 2020,
    price: 47250,
    mileage: 28750,
    bodyType: "Truck",
    fuel: "Diesel",
    transmission: "Automatic",
    location: "Boise, ID",
    color: "Forest Green",
    image: "https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Tow package", "Off-road suspension", "Apple CarPlay"],
  },
  {
    id: "cascade-convertible",
    make: "Cascade",
    model: "S-Line",
    year: 2019,
    price: 41875,
    mileage: 33400,
    bodyType: "Convertible",
    fuel: "Gasoline",
    transmission: "Automatic",
    location: "Miami, FL",
    color: "Crimson Red",
    image: "https://images.unsplash.com/photo-1523986371872-9d3ba2e2f642?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Premium leather", "B&O surround", "Adaptive cruise"],
  },
  {
    id: "orbit-compact",
    make: "Orbit",
    model: "City",
    year: 2022,
    price: 24800,
    mileage: 12500,
    bodyType: "Sedan",
    fuel: "Gasoline",
    transmission: "Automatic",
    location: "Portland, OR",
    color: "Cloud Silver",
    image: "https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Smartphone integration", "LED matrix lights", "City safety suite"],
  },
  {
    id: "summit-crossover",
    make: "Summit",
    model: "Altitude",
    year: 2023,
    price: 61200,
    mileage: 9800,
    bodyType: "SUV",
    fuel: "Hybrid",
    transmission: "Automatic",
    location: "Chicago, IL",
    color: "Obsidian Black",
    image: "https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?auto=format&fit=crop&w=1200&q=80",
    highlights: ["Luxury interior", "Tri-zone climate", "Hands-free driver assist"],
  },
];

window.getCustomInventory = function () {
  try {
    const stored = localStorage.getItem("customInventory");
    return stored ? JSON.parse(stored) : [];
  } catch (error) {
    console.warn("Unable to access localStorage", error);
    return [];
  }
};

window.saveCustomInventory = function (cars) {
  try {
    localStorage.setItem("customInventory", JSON.stringify(cars));
  } catch (error) {
    console.warn("Unable to save to localStorage", error);
  }
};
